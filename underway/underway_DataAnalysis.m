...
figure(1); clf; %Plot optode data showing spikes to remove
    subplot(2,1,1)
...
figure(5); clf; %Suggested nutrient samples to analyze
...
figure(6); clf; %Shows gps track with sections color coded
    ...
    axis([-42 -30 59.5 63]); legend(sectionL, 'Fontsize', 16);
...
figure(7); clf
    for i = 1:6
    subplot(4,1,1)
        scatter(ssw.time_filt(sections{i}), ssw.SST_filt(sections{i}), 15, sectionC{i}, 'filled'); hold on;
        axis([mintime maxtime SSTmin SSTmax]); datetick('x', 2, 'keeplimits'); title('Sea Surface Temperature', 'FontSize', 20); 
    subplot(4,1,2)
        scatter(ssw.time_filt(sections{i}), ssw.flr_filt(sections{i}), 15, sectionC{i}, 'filled'); hold on;
        axis([mintime maxtime flrmin flrmax]); datetick('x', 2, 'keeplimits'); title('Chlorophyll Fluorescence', 'FontSize', 20);
    subplot(4,1,3)
        scatter(optode.time_filt(sections{i}), optode.O2_nospike_salcorr_filt(sections{i}), 15, sectionC{i}, 'filled'); hold on;
        axis([mintime maxtime O2min O2max]); datetick('x', 2, 'keeplimits'); title('Oxygen Concentration', 'FontSize', 20);
    subplot(4,1,4) 
        scatter(suna.time_filt(sections{i}), suna.NO3_filt(sections{i}), 15, sectionC{i}, 'filled'); hold on;
        axis([mintime maxtime NO3min NO3max]); datetick('x', 2, 'keeplimits'); title('Nitrate', 'FontSize', 20);
    end
...
XtoPlot = [-5:25];
figure(8); clf; % Figure showing sections with NO3 vs AOU correlations as lines
for i = 1:6
    subplot(2,3,i)
...
for i = 1:4
    x = {(ssw.time_filt(OSNAPwest) - min(ssw.time_filt(OSNAPwest))), ssw.flr_filt(OSNAPwest), suna.NO3_filt(OSNAPwest), optode.aou_filt(OSNAPwest)};
    y = {'Days', 'Fluo', 'Nitrate', 'AOU'};
    figure(13); subplot(2,2,i);
        scatter(ssw.SST_filt(OSNAPwest), ssw.SSS_filt(OSNAPwest), [], x{i}, 'filled'); colorbar;
        xlabel('SST'); ylabel('Salinity'); title(y{i}); 
    figure(14); subplot(2,2,i);
        scatter(suna.NO3_filt(OSNAPwest), optode.aou_filt(OSNAPwest), [], x{i}, 'filled'); colorbar;
        xlabel('SST'); ylabel('Salinity'); title(y{i});
end