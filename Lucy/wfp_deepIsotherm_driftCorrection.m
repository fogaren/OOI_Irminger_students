%% Before running this section of code you will need to grid data on isotherms
%%%% To do this, you will need to go back to the mooring_extract_Irminger
%%%% script (that you used as a template for the HYPM data extraction and
%%%% depth gridding) and create a separate or parallel script to your
%%%% existing code for handling the Yr 1-4 data that will also grid on
%%%% isotherms rather than on depth surfaces.
...
for j = 1:length(thermplot)
    ...
    if length(idtherm) == 1
        ...
        ylabel ('[O_2]', 'Fontsize', 15)
        set(get(gca,'ylabel'),'rotation',0)
    end
end
datetick('x',3); ylim([274 304]); %legend(h, thermstr)
title([yearspan{i}], 'Fontsize', 17)
% subplot(4,2,i+4)
% for j = 1:length(thermplot)
%     idtherm = find(plotting.therm_grid == thermplot(j));
%     if length(idtherm) == 1
